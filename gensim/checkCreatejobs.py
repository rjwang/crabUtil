...
with open(allsubmits) as fp:
    lists = []
    for line in fp:
	if( ("[MC13TeV_" in line) or ("[Data13TeV_" in line) or ("[MC8TeV_" in line) or ("[Data8TeV_" in line) ):

	    line1 = line.split(']')[0]
	    setname = line1.split('[')[1]

	    ALLSCRIPT.writelines(' multicrab -create -cfg '+ 'Multicrab_'+setname+'.cfg \n')
	    ALLSCRIPT.writelines(' python ChangeXML.py -n 60 -c '+ setname+' \n\n')
	    ...
	    SCRIPT.writelines('CMSSW.output_file               = Events.root\n')
	    SCRIPT.writelines('CMSSW.total_number_of_events    = 150000\n')
	    SCRIPT.writelines('CMSSW.events_per_job            = 60\n')
