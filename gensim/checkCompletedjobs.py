...
for ajob in alljobs:
    SCRIPT = open('script_completedjobs_'+ajob+'.sh',"w")
	...
	if myfinishjobs == totaljobs[1]:
	  ...
	  SCRIPT.writelines('  multicrab -get 1-500  -c ' + ajob + ';\n')
	  SCRIPT.writelines('  mv '+mydir+'/res/Events*.root /tmp/rewang/'+mydir+'/res/'+';\n\n')

	  SCRIPT.writelines('  multicrab -get 1-1000 -c ' + ajob + ';\n')
	  SCRIPT.writelines('  mv '+mydir+'/res/Events*.root /tmp/rewang/'+mydir+'/res/'+';\n\n')

	  SCRIPT.writelines('  multicrab -get 1-1500 -c ' + ajob + ';\n')
	  SCRIPT.writelines('  mv '+mydir+'/res/Events*.root /tmp/rewang/'+mydir+'/res/'+';\n\n')

	  SCRIPT.writelines('  multicrab -get 1-2000 -c ' + ajob + ';\n')
	  SCRIPT.writelines('  mv '+mydir+'/res/Events*.root /tmp/rewang/'+mydir+'/res/'+';\n\n')

	  SCRIPT.writelines('  multicrab -get 1-2500 -c ' + ajob + ';\n')
	  SCRIPT.writelines('  mv '+mydir+'/res/Events*.root /tmp/rewang/'+mydir+'/res/'+';\n\n')
